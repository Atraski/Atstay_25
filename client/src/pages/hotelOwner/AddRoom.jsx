...
const [loading, setLoading] = useState(false);
  ...
  try {
    ...
    //Converting Anenities to Array & keeping only enabled Amenities
        ...
        <div className="grid grid-cols-2 sm:flex gap-4 my-2 flex-wrap">
          {Object.keys(images).map((key) => (
            <label htmlFor={`roomImage${key}`} key={key}>
              <img
                className="max-h-28 w-28 object-cover border rounded-md cursor-pointer opacity-80"
                src={
                  images[key]
  ...
  placeholder="0"
  min="0"
  className="border border-gray-300 mt-1 rounded p-2 w-24 appearance-none [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none"
  ...
  onChange={(e) => {
    ...
    // Ensure number and min 0
    setInputs({
      ...inputs,
      pricePerNight: Math.max(0, Number(value)),
    });
    }}
  />
